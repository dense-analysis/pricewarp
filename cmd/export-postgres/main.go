...
func exportUsers(conn *pgx.Conn, config exportConfig) error {
	...
	path := filepath.Join(config.outputDir, "crypto_users.csv")
...
func exportCurrencies(conn *pgx.Conn, config exportConfig) error {
	rows, err := conn.Query(context.Background(), "select ticker, name from crypto_currencies order by ticker")
	...
	path := filepath.Join(config.outputDir, "crypto_currencies.csv")
	writer, file, err := createCSV(path)
	...
	if err := writer.Write([]string{"ticker", "name", "updated_at"}); err != nil {
		return err
	}
...
func exportPrices(conn *pgx.Conn, config exportConfig) error {
	rows, err := conn.Query(
		...
		`
			...
			inner join crypto_currencies as from_currency
				on from_currency.id = price."from"
			inner join crypto_currencies as to_currency
...
func exportPortfolios(conn *pgx.Conn, config exportConfig) error {
	rows, err := conn.Query(
		...
		`
			...
			inner join crypto_currencies as currency
	...
	for rows.Next() {
		...
		if err := rows.Scan(&userID, &username, &ticker, &name, &cash); err != nil {
			return err
		}
...
func exportAssets(conn *pgx.Conn, config exportConfig) error {
	rows, err := conn.Query(
		...
		`
			...
			order by asset.user_id, currency.ticker
		`,
	)
